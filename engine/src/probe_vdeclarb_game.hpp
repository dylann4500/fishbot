...
public:
  ...
  // ---- P6 additions -------------------------------------------------------
  const Declaration* declNow = nullptr;   // verification: current round's proposals
  ...
  int pickWithin(int mode, const bool* want, const double* conf, int team) const {
    ...
    // Mode 5 (VERIFICATION ADDITION): ground-truth ORACLE arbitration.  Picks a
    // proposing seat whose named allocation is actually true, if one exists;
    // otherwise the lowest seat.  This is an upper bound on ANY within-team
    // arbitration rule, information-safe or not, and therefore a strictly
    // stronger control than confidence ranking for the question "what does the
    // information-safety constraint at game.hpp:205-209 cost?".
    if (mode == 5) {
      for (int i = 0; i < n; i++) { int p = order[i];
        if (want[p] && declTrue(p, declNow[p])) return p; }

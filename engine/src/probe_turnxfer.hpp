...
// Nothing here is on the shipped path; see probe_turnxfer_game.hpp for the
// patched driver.
#pragma once
#include "probe_turnxfer_game.hpp"
#include "factory.hpp"
#include "arena.hpp"
#include <thread>
...
inline void printCoord(const CoordStats& s, const CoordConfig& cc, std::ostream& os) {
  ...
  if (s.passMulti) {
    ...
    os << "   [V04-ONLY passers] multi decisions " << s.v4Multi
       << "   any candidate live " << s.v4AnyLive << " (" << pct(s.v4AnyLive, s.v4Multi) << "%)"
       << "   v0.4 pick live " << s.v4UniLive << " (" << pct(s.v4UniLive, s.v4Multi) << "%)\n";
    os << "   [V04-ONLY passers] mean sureRun v0.4 " << (s.v4Multi ? s.v4UniSure / s.v4Multi : 0)
       << "  ORACLE " << (s.v4Multi ? s.v4OraSure / s.v4Multi : 0)
       << "   DECISIVE " << s.v4Decisive << "   of which v0.4 chose the live one " << s.v4DecisiveOK << "\n";
    os << "   [V04-ONLY passers] shortfall hist:";
    for (int i = 0; i < 16; i++) if (s.v4LossHist[i]) os << "  " << i << "->" << s.v4LossHist[i];
    os << "\n";

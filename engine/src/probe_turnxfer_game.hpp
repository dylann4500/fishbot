...
struct CoordStats {
  ...
  // ---- ADVERSARIAL-VERIFY additions: the same comparison restricted to
  // decisions where the PASSER is a genuine V04Agent, so a mixed matchup does
  // not blend another agent's choosePassTarget into the "v0.4" column.
  long long v4Multi = 0, v4AnyLive = 0, v4UniLive = 0, v4Decisive = 0, v4DecisiveOK = 0;
  double    v4UniSure = 0, v4OraSure = 0;
  long long v4LossHist[16] = {0};
  ...
  void merge(const CoordStats& o) {
    ...
    v4Multi += o.v4Multi; v4AnyLive += o.v4AnyLive; v4UniLive += o.v4UniLive;
    v4Decisive += o.v4Decisive; v4DecisiveOK += o.v4DecisiveOK;
    v4UniSure += o.v4UniSure; v4OraSure += o.v4OraSure;
    for (int i = 0; i < 16; i++) v4LossHist[i] += o.v4LossHist[i];
  ...
  int selectPassTarget(const int* cand, int n) {
    ...
    if (cst) {
      ...
      if (n >= 2) {
        ...
        if (dynamic_cast<V04Agent*>(agents[g.turn])) {
          int qu = sureRun(uni);
          cst->v4Multi++;
          if (mx > 0) cst->v4AnyLive++;
          if (qu > 0) cst->v4UniLive++;
          cst->v4UniSure += qu; cst->v4OraSure += mx;
          if (mn == 0 && mx > 0) { cst->v4Decisive++; if (qu > 0) cst->v4DecisiveOK++; }
          cst->v4LossHist[std::min(15, mx - qu)]++;
        }

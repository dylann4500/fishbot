...
struct ArbStats {
  ...
  // --- VERIFICATION counters (added by the adversarial check) -------------
  long long vDeclTotal = 0;        // voluntary declarations executed (== rounds)
  long long vDeclInRace = 0;       // ... where the DECLARING team had >=2 proposers
  long long vDeclInContested = 0;  // ... and that team's conf-argmax != lowest seat
  long long vDeclInRaceWrong = 0;  // ... in-race declarations that were false
  long long vRaceDiffDecl = 0;     // races where candidates are NOT all identical
  long long vRaceOutcomeDiffers=0; // races where declTrue differs across candidates
  long long vRaceAllWrong = 0;     // races where EVERY candidate is false
  long long vRaceAnyRight = 0;     // races where at least one candidate is true
  long long vRace3 = 0;            // races with all 3 seats proposing
  long long vContestedDiffSet = 0; // contested races where low/cbest name diff sets
  long long vDeclInContestedWrong = 0; // executed decl in a contested round that was false
  long long vRace2 = 0, vRace2AllWrong = 0; // races with EXACTLY two proposers
  long long vContested2 = 0, vContested2BothWrong = 0;
  void merge(const ArbStats& o) {
    vDeclInContestedWrong += o.vDeclInContestedWrong;
    vRace2 += o.vRace2; vRace2AllWrong += o.vRace2AllWrong;
    vContested2 += o.vContested2; vContested2BothWrong += o.vContested2BothWrong;
    vDeclTotal += o.vDeclTotal; vDeclInRace += o.vDeclInRace;
    vDeclInContested += o.vDeclInContested; vDeclInRaceWrong += o.vDeclInRaceWrong;
    vRaceDiffDecl += o.vRaceDiffDecl; vRaceOutcomeDiffers += o.vRaceOutcomeDiffers;
    vRaceAllWrong += o.vRaceAllWrong; vRaceAnyRight += o.vRaceAnyRight;
    vRace3 += o.vRace3; vContestedDiffSet += o.vContestedDiffSet;
...
public:
  ...
  bool declarationRound() {
    ...
    for (int round = 0; round < NSET + 2; round++) {
      ...
      int win[2] = {-1, -1};
      bool vRaceTeam[2] = {false, false}, vContTeam[2] = {false, false};
      for (int t = 0; t < 2; t++) {
        ...
        if (nw >= 2) {
          arb.races++;
          vRaceTeam[t] = true;
          if (nw >= 3) arb.vRace3++; else arb.vRace2++;
          {
            // Does arbitration change WHAT is declared, and does it change the outcome?
            bool allSame = true, anyRight = false, allWrong = true, firstSet = true;
            bool ref = false;
            bool outDiff = false;
            for (int p = t; p < NPLAY; p += 2) if (want[p]) {
              bool tr = declTrue(p, decl[p]);
              if (tr) anyRight = true;
              if (firstSet) { ref = tr; firstSet = false; }
              else if (tr != ref) outDiff = true;
              if (p != lowest) {
                if (decl[p].set != decl[lowest].set) allSame = false;
                else { for (int i = 0; i < SETSZ; i++)
                         if (decl[p].owner[i] != decl[lowest].owner[i]) allSame = false; }
              }
            }
            allWrong = !anyRight;
            if (!allSame) arb.vRaceDiffDecl++;
            if (outDiff) arb.vRaceOutcomeDiffers++;
            if (allWrong) { arb.vRaceAllWrong++; if (nw == 2) arb.vRace2AllWrong++; }
            if (anyRight) arb.vRaceAnyRight++;
          }
          ...
          if (cbest != lowest) {
            vContTeam[t] = true;
            if (decl[cbest].set != decl[lowest].set) arb.vContestedDiffSet++;
            ...
            if (nw == 2) { arb.vContested2++; if (!lr && !cr) arb.vContested2BothWrong++; }
      ...
      arb.rounds++;
      {
        int ct = teamOf(chosen);
        arb.vDeclTotal++;
        if (vRaceTeam[ct]) {
          arb.vDeclInRace++;
          if (!declTrue(chosen, decl[chosen])) arb.vDeclInRaceWrong++;
        }
        if (vContTeam[ct]) { arb.vDeclInContested++;
          if (!declTrue(chosen, decl[chosen])) arb.vDeclInContestedWrong++; }
      }

...
const POLICY_ORDER: StrategyId[] = ["fishbot", "detective", "diversifier", "hunter", "bluffer", "random"];
...
function DecisionReadout({ action }: { action: GameAction }) {
  ...
  return <div className="decisionReadout">
    <div className="decisionSummary"><b>Why this ask?</b><p>{STRATEGIES[d.policy].name} selected it from all legal card–target pairs. FishBot estimated a <strong>{pct(d.features.hitProbability, 0)}</strong> hit probability, <strong>{fixed(d.features.informationGain)}</strong> bits of information, and <strong>{pct(d.features.replyThreat, 0)}</strong> opponent reply threat after a miss.</p></div>
    <div className="featureGrid">
      ...
      <span><small>FishBot utility</small><b>{fixed(d.fishbotScore)}</b></span>
      <span><small>Decision regret</small><b>{fixed(d.regret)}</b></span>
    </div>
    <div className="alternatives"><small>FishBot top candidates</small>{d.alternatives.map((alt, index) => <span key={`${alt.card}-${alt.target}`}><i>{index + 1}</i><b>{CARDS[alt.card].compact} from {PLAYER_NAMES[alt.target]}</b><em>{fixed(alt.score)}</em></span>)}</div>
  </div>;
}
...
export function FishLab() {
  ...
  const [teamB, setTeamB] = useState<StrategyId>("detective");
  ...
  return <main className="shell">
    ...
    <section className="labGrid">
      <aside className="configuration"><header><b>Experiment configuration</b><span>01</span></header><PolicySelect label="Team A policy" value={teamA} onChange={setTeamA} /><PolicySelect label="Team B policy" value={teamB} onChange={setTeamB} />
        ...
        <Toggle checked={tells} onChange={setTells} label="Ask-response evidence" detail="Public asks update card-location beliefs" /><Toggle checked={declarations} onChange={setDeclarations} label="Strategic declarations" detail="Policies declare from confidence, not hidden cards" />
    ...
    <section className="modelsSection" id="models"><header className="sectionHeader"><div><small>03 · DECISION MODELS</small><h2>What each computer actually does</h2></div><p>These policies are numeric and inspectable. “Reacting to an ask” means updating a defined belief state or applying a defined response weight—not sending prose to an LLM.</p></header><div className="fishbotNote"><b>FishBot v0.2 is the first search-based policy—not solved Fish.</b><p>For every legal ask, it estimates card-location probability, information gain, set progress, turn retention, and the target’s strongest reply after a miss. It takes the maximum expected-utility action. In 10,000 orientation-swapped games, the simpler detective still led 51.76% to 48.24%—evidence that perfectly optimizing a hand-built utility formula is not the same as optimizing long-run wins.</p></div><div className="policyList">{POLICY_ORDER.map((id, index) => { const policy = STRATEGIES[id], technical = POLICY_TECHNICAL[id]; return <details key={id} open={id === "fishbot"}><summary><span>{String(index + 1).padStart(2, "0")}</span><div><b>{policy.name}</b><small>{technical.class}</small></div><em>declaration ≥ {pct(policy.declarationThreshold, 0)}</em></summary><div className="policyBody"><section><small>OBJECTIVE</small><p>{technical.objective}</p></section><section><small>ASK SCORE</small><code>{technical.askFormula}</code></section><section><small>REACTION TO PUBLIC ASKS</small><p>{technical.reactsToAsk}</p></section><section><small>DECLARATION</small><p>{technical.declarationRule}</p></section><section><small>LIMITATION</small><p>{technical.limitations}</p></section></div></details>; })}</div></section>
    ...
    <footer><b>FishLab v0.2</b><span>Deterministic simulation · approximate hidden-information beliefs · reproducible seeds</span><a href="#lab">Back to experiment ↑</a></footer>

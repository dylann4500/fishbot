...
\providecommand{\vsevenHeadCommit}{}\renewcommand{\vsevenHeadCommit}{468580e}
...
\providecommand{\vsevenBuildDate}{}\renewcommand{\vsevenBuildDate}{2026-08-27}
% macros used in sections_v07 that this script generates from artifacts
\providecommand{\vsevenProvGenerated}{}\renewcommand{\vsevenProvGenerated}{244}
...
measures the principal comparison under \vsevenDiaRowsWord{} of them. The consequential choices are a
...
\S\ref{sec:agent-tiebreak} and \S\ref{sec:development-tie} use, because \fishseven{} modifies
...
{\tabfont\begin{tabular}{p{0.20\linewidth}p{0.24\linewidth}p{0.34\linewidth}l}
\toprule
...
\begin{equation}
  ...
  \label{eq:askscore}
\end{equation}
with $\lambda$ and $\beta$ scale parameters of the fit. Writing $p_a = \hat\mu(c,q)$ for the modelled
hit probability, the features $\phi$ are functions of the belief and the public state. They include
$p_a$ and $p_a^2$; an indicator that the card is publicly certain; the acting seat's holding in
$H(c)$ and the posterior mass its team already carries, each gated by $p_a$ so that a provably dead
ask cannot outscore a live one; the best continuation probability elsewhere in the half-suit; the
...
\fishseven{} inherits $w$, $\lambda$, $\beta$ and the three extra weights from \vsix{} unchanged, and
\textbf{no fit of any kind was applied during this cycle}. The vector was found by \vsix{}'s repaired
...
\begin{equation}
  a^\star \;=\; \mathcal{E}_t\bigl[\,\rho\bmod|\mathcal{E}_t|\,\bigr],
  \qquad
  \rho \;=\; \mathrm{hash}\bigl(\mathcal{P}_t,\; s,\; t\bigr),
  \label{eq:tiebreak}
\end{equation}
with $\mathcal{P}_t$ the running hash of the public event stream, $s$ the seat index and $t$ the
event count. Every seat observes all three arguments, so every teammate can recompute $\rho$; no
seat's private hand enters it. The mechanism is a coordination repair that happens to be free, and it
keeps the policy a deterministic function of $(\text{own hand}, \text{public record})$, which is the
condition the side-channel audit of \S\ref{sec:evaluation-side} tests.
...
The one component carrying measurable strength is a single additional coordinate, weighting how much
an ask hands to the opposing team when it \emph{misses}. A failed ask publishes a negative
certificate, and a negative certificate about a card the opposing team is itself unsure of helps them
resolve their own allocation. Let $O_H$ be the number of cards of half-suit $H$ whose posterior mass
sits with the opposing team, and $u_H$ the number whose holder this seat cannot place. The term is
\begin{equation}
  r(a) \;=\; (1 - p_a)\cdot\frac{O_{H(c)}}{6}\cdot\frac{u_{H(c)}}{6},
  \label{eq:contest}
\end{equation}
entering \eqref{eq:askscore} with a negative-signed fitted weight, so a large value discourages the
ask. The first factor is the probability the ask misses and therefore publishes the certificate; the
second is how much of the half-suit the opponents hold, so how much use the certificate is to them;
the third is how ambiguous the half-suit still is, since a certificate about an already-public card
tells nobody anything. In \fishseven{} the coefficient is set to
\texttt{\vsevenOptContest{}} (\S\ref{app:parameters}).

Two facts about its provenance matter for \S\ref{sec:results-attribution}. It was found by an
\emph{unfitted} coordinate sweep during adversary generation, not by architecture work: a single
hand-set value, with no fitting, measured at \vsevenOneSwitchArm{}~pp against the deployed policy over
\vsevenPhaseTwoCell{} games. And its edge against the more expensive frontier configuration \fmid{} is
\vsevenOneSwitchArmVsFmid{}~pp, which is negative, so it is a strength gain with a worst case rather
than an exploit (\S\ref{sec:development-adversaries}).
...
\begin{equation}
  ...
  \label{eq:bestalloc}
\end{equation}
with $\pi_{\mathrm{alloc}}$ from \eqref{eq:alloc}. The deployed path computes \eqref{eq:bestalloc}
sequentially from the marginals rather than by enumeration. \S\ref{sec:development-alloc} reports the
measurement that closed the question of whether that shortcut costs anything: over
$\sim$\vsevenLoneDecls{} recorded declarations it selects the same allocation as the exact joint
argmax every time, because the sequential rule's first chain-rule factor \emph{is} the marginal and
...
and $\pi_{\mathrm{team}}$ for the probability the team owns $h$ outright. Waiting is free in this
dialect, because an opponent cannot claim a half-suit the acting team holds, so the decision is not a
race but a trade: declaring scores a point now with probability $\pi_{\mathrm{alloc}}^\star$, and
...
\begin{equation}
  ...
  \label{eq:declev}
\end{equation}
and declares when $\Delta > \delta$ for a fitted margin $\delta$. The two conditioned states differ
in the score margin, by $+1$ and $-1$ respectively, and in having the six cards of $h$ removed from
the unresolved set.
...
on a threshold, $\pi_{\mathrm{alloc}}^\star \ge \tau$, and on a lower threshold once
$\pi_{\mathrm{team}} > 1 - 10^{-3}$, since a half-suit the team certainly owns will have to be
claimed eventually and delay only costs information. Under \emph{pressure}, the escalation described
next, the thresholds drop further. \fishseven{} removes the mechanism that used to raise pressure in
ordinary play.
...
Let $\chi_t = \mathrm{hash}\bigl(\mathcal{M}_t(\cdot),\, U_t,\, (\text{hand sizes})_t,\,
(\text{live half-suits})_t\bigr)$, the digest of exactly the certificates the seat holds. Define the
no-progress counter
\begin{equation}
  \nu_t \;=\;
  \begin{cases}
...
$P_t = \sum_{c \in U_t}\bigl(|\mathcal{M}_t(c)| - 1\bigr) + |U_t| + |\mathcal{H}_t|$. Certificates
...
of those routes therefore strictly decreases $P_t$, which is a non-negative integer, so there can be
...
determinizations from the exact posterior of \eqref{eq:posterior}, using the block dynamic program as
a uniform sampler over deals satisfying (C1)--(C4) with the (C5) disjunctions enforced by rejection,
so an accepted draw is an exact sample. The policy prior of \eqref{eq:prior} is then applied as an
...
candidates, with index $0$ reserved for the blueprint's own choice $a_{(1)}$. For each candidate $r$
and world $d$, all six seats are reconstructed \emph{at their own information sets} under $d$ and the
game is played to a terminal state or to depth \texttt{\vsevenOptDepth{}}, returning the acting
team's half-suit differential $V_{r,d}$. This is the point at which the method departs from standard
...
the blueprint's choice. The shrinkage is $\kappa = \texttt{\vsevenOptKappa{}}$, and $\eta$ is a small
blend back toward the blueprint's own ranking.
...
must be deterministic given $(\text{own hand}, \mathcal{P}_t)$, which is what
\eqref{eq:tiebreak} restores and what an unstable sort had broken. And $\Pi$ must be common
...
Every step reads only the acting seat's own hand and the public record. That is what makes the
coordination argument of \S\ref{sec:agent-coordination} go through, and it is the property the
...
\fishseven{} passes all \vsevenGateRulesWord{} rules: \vsevenGateDeadAsks\% provably dead asks against a
...
\label{tab:worstcase}
{\tabfont\begin{tabularx}{\linewidth}{@{}L l r c r@{}}
\toprule
...
{\tabfont\begin{tabularx}{\linewidth}{@{}L r c r c@{}}
\toprule
...
\midrule
0.05 \textit{(sub-floor)} & \ci{$+0.78$}{+0.25, +1.32} & \ci{$-3.62$}{-4.26, -2.99} & \ci{$-4.32$}{-4.94, -3.69} & \ci{$\mathbf{+0.70}$}{-0.20, +1.59} \\[0.4ex]
0.08 & \ci{$+1.59$}{+1.05, +2.13} & \ci{$-2.29$}{-2.92, -1.67} & \ci{$-4.67$}{-5.30, -4.05} & \ci{$\mathbf{+2.38}$}{+1.50, +3.26} \\[0.4ex]
0.11 & \ci{$+8.52$}{+7.93, +9.12} & \ci{$+2.52$}{+1.89, +3.16} & \ci{$-6.90$}{-7.52, -6.28} & \ci{$\mathbf{+9.42}$}{+8.54, +10.31} \\[0.4ex]
0.15 & \ci{$+11.44$}{+10.85, +12.03} & \ci{$+7.30$}{+6.68, +7.93} & \ci{$-5.34$}{-5.97, -4.71} & \ci{$\mathbf{+12.64}$}{+11.75, +13.53} \\[0.4ex]
\bottomrule
\end{tabularx}}
\end{table}
...
\noindent
No fit of any kind was applied to the frozen configuration in this cycle
...
{\tabfont\begin{xltabular}{\linewidth}{@{}L l r r r r@{}}
\toprule

...
planning or a working learner for it, and no coverage in the standard card-game reinforcement
learning libraries (\S\ref{sec:related-fish}). The nearest published agent plays a four-player,
...
\begin{description}
\item[RQ1] Does \fishseven{} achieve a replicated improvement over the strongest relevant
  configuration available from \vsix{}, measured on material sealed before \fishseven{} existed?
...
\item \textbf{Negative and diagnostic results} (\S\ref{sec:development-rejected},
  ...
  measurable improvement, and identifying a resolution barrier: the gains that remain in this policy
  class, if any, are smaller than the evaluation and adversary-search machinery can reliably detect.
\end{enumerate}
...
opposing team scores it. A player may not declare a half-suit they hold entirely, so the interesting
case is always a claim about cards in teammates' hands.
...
on \cite{develin} and the rules of record supplied by the project owner. Every contested choice is an
engine switch, so its effect is measurable rather than assumed, and \S\ref{sec:results-dialects}
measures the principal comparison under \vsevenDiaRows{} of them. The consequential choices are a
...
A deal resolves in roughly a hundred public events, and the engine plays hundreds of complete
six-player games per second per core with exact inference running in every seat. This is what makes
...
opponent panel scored identically for four configurations, and \vsevenBfourArms{} independent
adversarial searches all run on one desktop within a day. In a domain where one evaluation cell costs
...
and durations \cite{goadrich2026valet}, though its abstract does not state whether any of the twenty-one is a team game, so no
comparison is drawn against it here.
...
\S\ref{sec:inference}. No learned belief model appears anywhere in this corpus, and that is a
...
{\small\begin{tabular}{p{0.20\linewidth}p{0.24\linewidth}p{0.34\linewidth}l}
\toprule
...
Termination guarantee & the urgency clock & deduction-state stall detector (\texttt{stall}) & \S\ref{sec:agent-stall} \\
...
A configuration is therefore gated \emph{before} any strength number is computed for it, on eight
...
\vsevenTieContested\% of contested ask decisions tie bit-for-bit, and the open question was whether
anything can be \emph{selected} within that set rather than merely randomised. A learned re-ranker
...
\begin{table}[t]\centering
\caption{Every configuration this paper compares. Configurations marked as pre-existing were
available before the v0.7 programme began; the phase-2 composite was assembled during it. Only
\fishseven{} was frozen for holdout evaluation.}
\label{tab:configs}
\small
\begin{tabular}{lp{0.30\linewidth}lp{0.26\linewidth}}
\toprule
...
\composite{} & v0.6 frontier search, plus the contestation weight, plus one switch & assembled in
development phase 2 of this cycle & the strongest configuration available before the architecture
work; the comparison in \S\ref{sec:results-composite} \\
...
\node[st] (dev)  at (3.6,0)  {\textbf{development}\\{\scriptsize instrument, adversaries,}\\[-1pt]{\scriptsize candidate mechanisms}};
\node[st] (sel)  at (7.2,0)  {\textbf{selection}\\{\scriptsize $K$ configurations}\\[-1pt]{\scriptsize scored and compared}};
\node[st] (frz)  at (10.8,0) {\textbf{freeze}\\{\scriptsize one configuration,}\\[-1pt]{\scriptsize artifact round-tripped}};
\node[seal] (ev) at (14.4,0) {\textbf{evaluation}\\{\scriptsize sealed holdout,}\\[-1pt]{\scriptsize registered protocol}};
\draw[arr] (base) -- (dev); \draw[arr] (dev) -- (sel); \draw[arr] (sel) -- (frz); \draw[arr] (frz) -- (ev);
\node[align=center,font=\scriptsize,fishgray,text width=76mm] (train) at (5.4,-1.75)
...
\node[align=center,font=\scriptsize,fishgreen,text width=42mm] (hold) at (14.4,-1.75)
  {\textbf{sealed material}: banks committed by digest, adversary half encrypted, decoded only here.};
\draw[fishgreen,very thick,dashed] (12.6,1.2) -- (12.6,-2.6);
\node[font=\scriptsize\bfseries,fishgreen,rotate=90] at (12.35,-0.7) {seal};
\draw[fishgray,dotted] (5.4,-0.75) -- (5.4,-1.15);
\draw[fishgreen,dotted] (14.4,-0.75) -- (14.4,-1.15);
\end{tikzpicture}
...
\toprule
Rule & FishBot v0.7 & v0.6 & \texttt{F-cheap} & Negative control \\
\midrule
...
\begin{figure}[t]\centering
{\small\begin{tikzpicture}[x=1cm,y=1cm]
...
\begin{table}[t]\centering
\caption{Worst panel cell per configuration.}
\label{tab:worstcase}
...
\toprule
Far archetype & FishBot v0.7 & v0.6 & \texttt{F-cheap} & Composite \\
\midrule
...
gain, checkable as a leave-one-out drop of roughly two points. On that check it holds: the holdout
...
copies of the named partner; team B is three copies of the opponent.}
\label{tab:partners}
...
\begin{table}[t]\centering
\caption{Partner-regime table against a \vsix{} opponent.}
...
from the banks it is evaluated on.}
...
\vsevenBfourArms{} were evaluated. \textbf{None of the \vsevenBfourArms{} searches found a positive
...
they maximise win rate as four of the eight adversarial searches do, so this is a second sample
...
\midrule
FishBot v0.7 & 7090001 & 0 / 813,512 & \textbf{0} / 813,512 \\
FishBot v0.7 & 7090002 & 3 / 816,049 & \textbf{0} / 816,016 \\
...
\midrule
FishBot v0.7 & 4.52$\times$ & 4.72$\times$ & 2.72$\times$ \\
...
moves the result by \vsevenXpGapAbs{}~pp; and the advantage survives \vsevenDiaRows{} rule dialects
...
$[\vsevenLooRtwelveLo, \vsevenLooRtwelveHi]$. That component originated in an unfitted coordinate
sweep during adversary generation rather than in architecture work, and its provenance matters for
the next subsection.
...
in a new domain. Fitting against any self-oriented per-decision objective produces a worse policy
while moving its own proxy in the intended direction, because the ask and declaration channels trade
against each other. And nothing learned survives a free public-history tie-break inside the
exchangeable tied set, whose selection signal is approximately \vsevenCurseTieShare\% winner's curse
against approximately \vsevenCurseShare\% for the search as a whole.
...
None of the \vsevenBfourArms{} adversarial searches found a positive edge against \fishseven{} at the
...
generations of \vsevenBfourPop{}. Its failure is evidence about that search. This is the precise
...
the result by \vsevenXpGapAbs{}~pp; the advantage persists across \vsevenDiaRows{} rule dialects; and
none of \vsevenBfourArms{} independently constructed adversarial searches, including a white-box
...
but it was produced during adversary generation by an unfitted coordinate sweep, and the architecture
work that followed added no measurable strength on top of it. The second is that four plausible
...
\fishseven{} beats \fcheap{} on both primary holdout banks, pooled over \vsevenBtwoFcheapN{} games.
...
\noindent Result: the comparison \textbf{\vsevenPrimaryVerdict{}}.
...
S6 & the worst case is not catastrophic & \fishseven{}'s worst panel cell no worse than the deployed \vsix{} policy's & PASS \\
...
\fishseven{} as brittle in that named way. A failure of S7 would have been procedural and the evaluation would have
stopped.

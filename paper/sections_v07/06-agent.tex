...
What the policy actually enumerates is a strict subset of \eqref{eq:legal}. A \emph{live-ask filter}
first removes candidates the seat can already prove are misses, that is, those with
$s \notin \mathcal{M}(c)$ ruled out by a certificate, and it is this filter rather than any weighting
inside \eqref{eq:askscore} that prevents a provably dead ask from being chosen. The filter binds:
it removes what would otherwise have been the argmax at \vsevenGateBindRate\% of decisions.
...
count, and $\iota$ an unbiased index map from the hash into the tied set. Every seat observes all three arguments, so every teammate can recompute $\rho$; no seat's private
...
$\prod_{c} \hat\mu(c,\alpha(c))$, reporting that product as $\pi^\star_{\mathrm{alloc}}$. The exact
...
$\sim$\vsevenLoneDecls{} recorded declarations it selects the same allocation as the joint argmax every time, because the sequential rule's first chain-rule factor \emph{is} the marginal and
...
\begin{equation}
  ...
  \label{eq:declev}
\end{equation}
and declares when $\Delta > \delta$ for a fitted margin $\delta$. The two conditioned states differ in the score margin, by $+1$ and $-1$ respectively; in having the
\emph{unresolved} cards of $h$ removed from the unresolved set, typically fewer than six; and in the
deltas of the four half-suit-control features the value function reads, all of which are functions of
the team's expected share of $h$ before the claim.
...
sets} under $d$, with a cheaper belief than the acting agent's own and the game is played to a terminal state or until \texttt{\vsevenOptDepth{}} further public events
have elapsed. A run reaching the end returns the acting team's half-suit differential; a run cut at
the depth limit returns a learned leaf evaluation of the position instead. Write $V_{r,d}$ for
either. This is the point at which the method departs from standard

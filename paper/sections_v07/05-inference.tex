...
$s$ & the acting seat; $T(s)$ its team, $\mathcal{T}_s$ its two teammates, $\mathcal{O}_s$ the three
  opponents \\
...
block assignments whose image lies in $\mathcal{T}$. And the probability of a fully named allocation,
...
disjunction: for a certificate saying that seat $A$ held \emph{some} card of a half-suit, each
candidate card $d$ of that half-suit is reweighted by
$\hat\mu(d,A) \big/ \bigl(1 - \prod_{e}(1 - \hat\mu(e,A))\bigr)$ over the half-suit's cards $e$.
